...
public class AllResourcesTest extends AbstractCorePluginTest {
    ...
    public void test() {
        ...
        Map<String, Long> usersByIp = new HashMap<>();
        ...
        process(Application.class, it -> {
            ...
            if (it.getResourceName().startsWith("infra_url_redirection") || it.getResourceName().startsWith("infra_web")) {
                String json = JsonTools.compactPrint(it.getApplicationDefinition());
                json = json.replaceAll("70012", usersByIp.get("infra_url_redirection").toString());
                json = json.replaceAll("70013", usersByIp.get("infra_url_redirection").toString());
                it.setApplicationDefinition(JsonTools.readFromString(json, IPApplicationDefinition.class));
                changes.resourceUpdate(it);
            }
        });
